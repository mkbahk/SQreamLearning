...
select show_locks();
select show_db_info('seoulsmallbiz');

---get statement_id
select show_server_status();
---Query Plan을 보여줌
select show_node_info(4779);

select show_server_status();
select stop_statement(5228);

select show_connections();
select connection_stop_statement(3988);
---에러발생, 메뉴얼에도 안나옴(2022/03/21)

select data_diag(1,'/sqreamdb/sqream_storage/databases/seoulsmallbiz/tables/6/0/0-9997');
select data_diag(12,'/sqreamdb/sqream_storage/databases/seoulsmallbiz/tables/6/0/0-9997');
--can not remove key, file exists, data_diag()는 SQream문서파일에 검색이 않됨

select * from sqream_catalog.tables where table_name='salesforcasting';

select 1 from public.salesforcasting;
select 1;
select count(*) from sqream_catalog.md_deleted_files;

select export_leveldb_stats('/tmp/mkbahk_exports_leveldb_states.txt');
select reset_leveldb_stats();
select export_statement_queue_stats('/tmp/mkbahk_export_statement_queue_stats.txt');

select get_open_snapshots_statements('/tmp/mkbahk_opensnapshot_stats.txt');

select get_statement_locks($$select base_year_code, sum(quarterly_sales_amount), avg(quarterly_sales_amount) from salesforcasting group by base_year_code order by base_year_code desc$$);

select count(*) from sqream_catalog.chunks;

select get_license_info();

select show_server_status();
select show_instance_services('worker_5001');

select key_evaluate('view_light','seoulsmallbiz','public.salesforcasting');

select show_version();

select data_diag(1, 'seoulsmallbiz','public.salesforcasting');
select data_diag(12, '/sqreamdb/sqream_storage/databases/seoulsmallbiz/tables/6/0/0-9995');

select rows_count_correction('public','salesforcasting','correct');

select cleanup_chunks('public','salesforcasting');
---Error executing statement: value '1001238' is too long for column 'business_district_code@vcblob' of type TEXT(7)

select count(*) from salesforcasting where business_district_code = '1001238';
---783960
select cleanup_extents('public','salesforcasting');
...
SELECT t.table_name FROM sqream_catalog.delete_predicates dp
   JOIN sqream_catalog.tables t
   ON dp.table_id = t.table_id
   GROUP BY 1;
---Identify predicates for clean-up
SELECT delete_predicate FROM sqream_catalog.delete_predicates dp
   JOIN sqream_catalog.tables t
   ON dp.table_id = t.table_id
   WHERE t.table_name = 'salesforcasting';
